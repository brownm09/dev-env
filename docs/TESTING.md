...
88. **journal-shell-write-guard test** — required when changing `claude/scripts/pre-tool-use-journal-shell-write-guard.py` ([ADR-129](adr/129-journal-shell-write-guard.md), dev-env#961). Two layers: pure-function tests against the imported module (`journal_path_kind` classification for all four path shapes, `find_bash_redirect_targets`, `find_powershell_write_targets`, `find_journal_shell_writes`, `_has_override`), plus subprocess end-to-end tests driving the real hook over stdin.

    Notable pins: `test_find_bash_redirect_targets_heredoc_declaration_line` is the single most important test in the suite — it pins the `_mask_first_line_quotes` fix for the classic `cat <<'EOF' > <journal-file>` shape (the exact reported failure pattern). Fed directly to `_hookio.mask_quoted_spans` without first neutralizing `<<`, `_find_heredoc_end` treats an already first-line-truncated string as an unterminated heredoc declaration and masks the redirect target right along with it — traced and manually verified while authoring the hook; the fix is `<<` → `<#` neutralization (same length, so offsets stay aligned) before masking. `test_find_bash_redirect_targets_ignores_quoted_gt_in_prose` and `test_find_powershell_write_targets_value_argument_not_matched_as_target` pin that a `>` or a path-shaped word inside a quoted value/`-Value` argument is never mistaken for a real operator or target. `test_find_journal_shell_writes_heredoc_body_mention_not_triggered` pins the mirror-image case from the two sibling PreToolUse hooks: a heredoc *body* merely mentioning a journal filename as prose must not trigger (only the first physical line of a segment is ever inspected). `test_find_journal_shell_writes_git_add_commit_push_not_triggered`, `_rm_remove_item_not_triggered`, `_plain_reads_not_triggered`, and `_mkdir_new_item_directory_not_triggered` together pin every documented-legal operation on these paths (staging/committing/pushing the path as an argument, the documented shard-deletion mechanism, plain reads, and directory-only scaffolding) as explicitly out of scope — only a genuine redirect operator or one of five named PowerShell cmdlets is ever inspected. `test_main_no_cwd_in_payload_still_works` pins that this hook, unlike its two structural siblings, needs no cwd/git resolution at all — a payload omitting `cwd` entirely still blocks correctly, since detection is pure text/regex work over the command string.
