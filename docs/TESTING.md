...
78. **worktree-recovery recipe + runbook-parity gate** — required when changing `claude/scripts/_worktree_recovery.py`, the orphan block message in `claude/scripts/pre-tool-use-worktree-path-check.py`, or the `docs/REFERENCE.md` "Worktree deregistration recovery" runbook (dev-env#862, [ADR-116](adr/116-single-source-worktree-recovery-recipe.md)). Three layers, all pure offline file parse — no subprocess, network, or git. **(1) Rendering:** `recovery_recipe()` emits every `RECOVERY_STEPS` command in order with the `<canonical>`/`<orphan>` path placeholders substituted, leaves `<branch>` unsubstituted (the hook cannot read an orphan's HEAD), and stays `.isascii()` — the reason crosses Claude Code's cp1252 exit-2 stderr pipe (hook authoring rules 4/5). Also pins the load-bearing *ordering*: non-destructive `worktree repair` first, content-destroying `find -delete` last, so a reordering cannot silently cost a session its uncommitted files. **(2) Runbook parity:** the REFERENCE.md runbook's fenced block carries the same commands in the same order. The section scanner is deliberately fence-aware — the runbook's own `# 1. …` shell comments are indistinguishable from `#` headings by prefix, and a fence-blind scan ends the section early and yields an empty code block, i.e. the parity check would pass **vacuously**; that failure mode was hit and fixed during authoring, so treat any change to `_section()` as safety-critical. **(3) Anti-regression:** the dev-env#751-disproven `worktree add --force` form appears in no *emittable* string literal across `claude/scripts/*.py` (AST-based — comments never reach the AST and docstrings are excluded, so prose explaining *why* the form fails is exempt by design, while a prescription is caught), nor in the runbook code block or the hooks-table row. `docs/adr/` is deliberately exempt as a historical record: ADR-024's 2026-06-06 addendum legitimately quotes the then-believed-correct recipe, and ADR-116 supersedes it rather than rewriting it. The recipe itself was re-derived from a throwaway-fixture matrix against git 2.37.1.windows.1 rather than inherited — see ADR-116 for the evidence table, including that `worktree repair` can exit 1 while having succeeded and that plain `add` does *not* tolerate a non-empty target (correcting dev-env#862's own report).
