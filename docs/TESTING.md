...
9. **worktree-npm-install gate test** — required when changing `claude/scripts/worktree-npm-install.py`.
   ...
   **Several thresholds here are calibration constants, so the tests pin the measurement rather than
   the arithmetic.** `test_measured_benign_ceiling_stays_ok` asserts against `MEASURED_BENIGN_CEILING`
   = 21.3% — the worst *benign* empty-shell ratio observed across the 48-tree corpus, produced by the
   optional platform dependencies npm correctly skips — and separately asserts the floor still sits
   above it. A test that only re-checked `0.50 >= 0.50` would pass just as happily after an edit slid
   the floor down into the benign band, which is the regression that actually matters.
   ...
   **`test_scan_counts_staging_instead_of_partials` guards a false positive observed live, not
   ...
   (`.bin`, `.cache`, `.vite-temp`, `.package-lock.json`) that must *not* be read as staging.
   ...
   **Deliberate scope gap.** `_audit_existing_tree()` is not unit-tested: it shells out to `npm ci`
   and writes sentinels under the real `~/.claude/scratch`, both of which this suite's convention keeps
   out. Its decision logic lives entirely in the pure helpers above. Its end-to-end behaviour was
   verified before merge against live trees — `repair` on the confirmed known-bad
   (`reverent-kowalevski-79b384`, carrying the `@langchain/core` truncation dev-env#945 named) and `ok`
   on known-good trees carrying each benign class. `test_scan_skips_workspace_links` degrades to a
   printed note (still passing, covering less) where the platform refuses symlink creation — Windows
   without Developer Mode or elevation; the link branch is still covered purely by
   `test_classify_covers_the_four_shapes`.
...
78. **worktree-recovery recipe + runbook-parity gate** — required when changing `claude/scripts/_worktree_recovery.py`, the orphan block message in `claude/scripts/pre-tool-use-worktree-path-check.py`, or the `docs/REFERENCE.md` "Worktree deregistration recovery" runbook (dev-env#862, [ADR-116](adr/116-single-source-worktree-recovery-recipe.md)). Three layers, all pure offline file parse — no subprocess, network, or git. **(1) Rendering:** `recovery_recipe()` emits every `RECOVERY_STEPS` command in order with the `<canonical>`/`<orphan>` path placeholders substituted (in a **single pass** — chained `.replace()` is order-dependent and could rewrite a canonical path containing the literal `<orphan>` into the destructive step), leaves `<branch>` unsubstituted (the hook cannot read an orphan's HEAD), and stays `.isascii()`. That ASCII constraint is kept for the **raw** exit-2 stderr channel this hook moves to under dev-env#865 — today's `_block()` uses `json.dumps`, whose default `ensure_ascii=True` would already escape non-ASCII on the wire, so the constraint is insurance rather than a live requirement. Also pins the load-bearing *safety* invariants on the `destructive`/`conditional` flags rather than on list position: `worktree repair` is first, every destructive step is `conditional`, a salvage/capture step precedes the first destructive one, and no destructive step is rendered as a numbered item — the failure a position-only test misses is a stressed reader executing a numbered list top-to-bottom, not a reordering. **(2) Runbook parity:** the REFERENCE.md runbook fence's *runnable* lines must **equal** `RECOVERY_STEPS`' commands in order (modulo the named `RUNBOOK_ONLY_COMMANDS` allowlist), and must not reintroduce `rm -rf` or `checkout main`. Equality over runnable lines, not substring containment: the original substring form let **four** separate drift mutations pass — every step commented out, `rm -rf`/`checkout main` injected, a step swapped while its text survived only in a `# (formerly: …)` comment, and the destructive step hoisted to first. The section scanner is also deliberately fence-aware — the runbook's own `# 1. …` shell comments are indistinguishable from `#` headings by prefix, and a fence-blind scan ends the section early and yields an empty code block, i.e. the parity check would pass **vacuously**. Both failure modes were hit and fixed during authoring, so treat `_section()`, `_runnable_lines()`, and the equality assertion as safety-critical. **(3) Anti-regression:** the dev-env#751-disproven `worktree add --force` *or* `-f` form (both verified to fail identically) must not appear as a prescription. Two complementary passes, **neither claimed exhaustive**: an AST pass over non-docstring string literals in `claude/scripts/*.py` — comments never reach the AST, so prose explaining *why* the form fails is exempt by design while a prescription is caught — and a text pass over runnable fenced lines in the live docs, skills, and routines. Known, deliberate gaps: the AST pass does not see `+`/`%`/`.format()`/f-string-variable/`bytes`/`join()` constructions, and neither pass scans `claude/scripts/tests/`, since a test asserting the form is *absent* necessarily contains it. The matching regex bounds the verb→flag gap and excludes backticks and pipes, so it cannot span a markdown code-span or table-cell boundary — an unbounded version matched 874 characters across a whole hooks-table row. `docs/adr/` is exempt as a historical record: ADR-024's 2026-06-06 addendum legitimately quotes the then-believed-correct recipe, and ADR-116 supersedes it rather than rewriting it. The recipe itself was re-derived from a throwaway-fixture matrix against git 2.37.1.windows.1 rather than inherited — see ADR-116's evidence table, including that `worktree repair` exits 1 on success *and* on failure with the same message (so step 2 is the only reliable signal) and that plain `add` does *not* tolerate a non-empty target (correcting dev-env#862's own report).
...
81. **new-day-journal-check day-rollover + stale-canonical-recovery test** — required when changing `claude/scripts/new-day-journal-check.py` (ADR-119, amended for dev-env#911). Pins the pure helpers behind check 4 (day rollover): `branch_date` (parses `draft/<date>`, including the documented `-recovery` and other suffixed forms that exist on the remote today; returns None for `main`, feature branches, and malformed/unpadded dates, so no false rollover fires) and `mismatched_stub_paths` (deliberately one-sided — only a stub dated *after* the branch is the rollover failure; older-dated stubs are branch-lineage artifacts already covered by `stale_draft_artifacts`/`unmerged_draft_branches`, and including them buried the 6 real hits under 27 unrelated ones on the first live run). Also pins that non-convention filenames — the ones compose's own glob would not match either — are ignored, and that a branch on its own date reports nothing. The git-reading helpers (`canonical_current_branch`, `branch_stub_paths`) and `day_rollover_message` are subprocess boundaries, left untested per this repo's fixture-only convention.

    Check 5 (stale-canonical self-healing, ADR-119 Amendment 1) gets the same pure-function treatment for `stale_canonical_recovery_decision` (named `_decision`, not `format_*` like its sibling `format_day_rollover`, since a non-None return here also authorizes the checkout, not just advisory text — a PR #912 review finding) — silent on today's branch, a non-draft branch, any dirty tree regardless of idle time, under-threshold idle, and unknown idle time; fires only when clean AND idle >= `STALE_CANONICAL_IDLE_MINUTES` (15); a regression pin asserts dirty short-circuits before idle_minutes is even consulted. Unlike every other check in this file, check 5's action is a REAL `git checkout main`, not just advisory text, so it ALSO gets five real-repo end-to-end tests — the one deliberate departure from this item's "subprocess boundaries are left untested" convention, justified because only a real repo can prove the single most important safety property: a dirty tree is never actually checked out. These drive the real `stale_canonical_recovery_message()` against a disposable throwaway git repo (borrowing the init-a-real-repo fixture technique from `test_journal_canonical_guard.py`'s `_init_throwaway_repo`, backdating commits via `GIT_AUTHOR_DATE`/`GIT_COMMITTER_DATE` instead of sleeping) and assert on the repo's actual branch afterward: today's branch is never touched even when artificially idle; a dirty stale branch is never checked out (and the uncommitted change itself survives untouched); a recently-committed (<15min) stale branch is left alone; **a branch with an ancient tip commit but JUST checked out is left alone** (the PR #912 review regression pin — see `canonical_head_idle_minutes`'s docstring for why idle time is measured from HEAD's own reflog, not the branch's tip-commit time); and a clean, sufficiently idle stale branch is actually restored to `main`. The test runner's per-test exception handling was widened from `except AssertionError` to also catch generic `Exception` (matching `test_journal_canonical_guard.py`'s pattern) so an unexpected real-git-subprocess failure in one of these fixtures reports as a single clean `ERROR:` line instead of aborting the rest of the suite.

    ```bash
    py -3 claude/scripts/tests/test_new_day_journal_check.py
    ```

82. **composed-output stray-terminal-scan test** — required when changing `claude/scripts/_composed_output_scan.py` or `claude/scripts/validate-composed-output.py` ([ADR-121](adr/121-composed-output-stray-terminal-scan.md), dev-env#894). Pure offline string scan — no disk, network, or subprocess. Pins the **real** 2026-07-11 corruption (engineering-journal#183) verbatim as a fixture, asserting each check independently rather than "some finding exists": the non-indented `Please specify which branch` / `See git-rebase(1)` lines trip `signature` (lines 4, 5, 11) while the indented command lines trip `progress-summary-indent` (lines 7, 11), so a regression in either check still leaves the incident caught. Also pins that the finding carries the **full** offending line, since the welded-on real sentence (`ADR count now at 101+.`) is exactly what blind deletion would destroy. Exemptions are pinned against the three real false-positive sites that made fence-awareness mandatory (`career-playbook/2026-06-21-…:75`, `dev-env/2026-07-02-…:392`, `dev-env/2026-07-13-…:190` all quote `fatal:` inside fences), plus the **meta case** — a journal *about* this bug names the signatures in inline code spans, including a ``` ``double-tick span containing `single` ticks`` ``` that a naive ``` `[^`]*` ``` regex splits and leaks. Fence mechanics: info strings, tilde fences, `~~~` inside a ``` block not closing it, a shorter closer not closing a longer opener, an unclosed fence running to EOF (CommonMark), and the delimiter lines themselves counting as fenced. Anchoring: `fatal: `/`hint: ` match only at line start, so "Git printed the fatal: line" is not a hit. Section bounds: the indent check starts at `## Progress Summary`, stops at the next heading of any level, reopens on a second such heading, and ignores blank indented lines.
...
84. **README index-parity gate** — required when changing `claude/scripts/tests/test_readme_index_parity.py`, or when adding/removing/renaming any file in `claude/scripts/tests/` or `claude/scripts/` ([ADR-122](adr/122-directory-index-readme-parity-gate.md), dev-env#901). Pure offline file parse — no subprocess, network, or git. The directory<->README analogue of item 76 (`test_testing_index_parity.py`, which gates `CLAUDE.md` <-> `docs/TESTING.md`): here the two hand-maintained per-file indexes `claude/scripts/tests/README.md` (dev-env#822) and `claude/scripts/README.md` (dev-env#830) are checked against the directories they claim to index. Neither directory's tooling reads its README — `run-hook-tests.py` discovers tests by glob and Claude Code loads scripts by their `settings.json` wiring — so a file with no row still works and the index rots invisibly; by the time the gate was written both had drifted (missing rows, plus header counts that were stale and, in `claude/scripts/README.md`, self-contradicting: it stated both "79 files" and "76 files" for the same 82-file set).
    ...
    Six checks per indexed directory, table-driven over a per-directory `DirSpec` (directory, README, first-column exemptions, header count sentences, expected number of `(N)` headings, and optional component globs), each README parsed once in `setUpClass` (matching the item-76 sibling):
    ...
    4. **Header counts** — each header count sentence occurs **exactly once** and equals the live directory count (`claude/scripts/tests/`: `test_*.py`, bash-gate, and total; `claude/scripts/`: top-level total). Exactly-once, not first-match, so a stray second copy of a count phrase can't hide drift behind it.
    5. **Section counts** — every `### Title (N)` heading's N equals that section's first-column entry count, **and** the number of `(N)` headings matches a per-directory expectation (`claude/scripts/README.md`: 5; `claude/scripts/tests/README.md`: 0) so a heading reworded past the `(N)` form can't silently drop out of gating.
    6. **Component completeness** — where a header breaks its total into components (the tests README's "N `test_*.py`, N bash gates, one module"), every indexable file matches a declared component glob or is an exemption, so a file of a new kind (e.g. a stray `.ps1`) can't make the components silently fail to sum to the gated total.

    Fail-closed by construction: a count sentence reworded past its regex fails check 4 (zero matches), a duplicated count fails it too (more than one), and a numbered heading that loses its `(N)` fails check 5's heading-count assertion — none pass on a count the gate can no longer see, the correct direction for a gate whose whole job is catching silent drift. This is why the two headers are worded so the numbers parse without depending on an em-dash. Verified during authoring that an injected unindexed test file trips checks 1 and 4 together (missing row + both stale counts, exit 1), that rewording a numbered heading drops the numbered-heading count 5→4 (check 5), and that a `.ps1` added to the tests dir is reported uncovered (check 6).

    ```bash
    py -3 claude/scripts/tests/test_readme_index_parity.py
    ```

85. **skill-file-size-guard test** — required when changing `claude/scripts/pre-tool-use-skill-file-size-guard.py` ([ADR-127](adr/127-skill-file-size-guard.md), dev-env#939). Two layers: pure-function tests against the imported module (`_is_skill_md` case-insensitive basename matching, `resulting_write_size` UTF-8 byte counting vs. char counting, `resulting_edit_size`'s substitution logic, `current_file_size`, `load_limit_bytes`'s config fallback matrix), plus subprocess end-to-end tests driving the real hook over stdin.

    Notable pins: the hard-limit boundary is strictly greater-than — `test_write_at_exactly_limit_bytes_passes` and `test_write_one_byte_over_limit_blocks` both exercise the exact edge, since a limit is a ceiling a file may legitimately reach, not a value that itself blocks. The block predicate compares against BOTH the limit and the file's current on-disk size (`size > limit AND size > current_size`) — `test_edit_shrinks_but_stays_over_limit_passes` starts from an oversized fixture and shrinks it by a genuine amount that still leaves it over the limit, and must NOT block (an oversized file must be trimmable incrementally across multiple edits, not just in one all-at-once edit); `test_edit_grows_already_oversized_file_still_blocks` confirms the shrink allowance isn't a blanket bypass for already-oversized files. `resulting_edit_size` compares and substitutes on a line-ending-normalized copy of `current`/`old_string`/`new_string`, then re-applies the file's own line ending when measuring the final size: `test_resulting_edit_size_multiline_old_string_matches_crlf_file` pins the regression this fixes directly — a real bug found by `/review`, verified against the real `Edit` tool, where a byte-literal comparison never matched a multi-line `old_string` against a CRLF file, so the guard silently never enforced the limit on any multi-line CRLF edit; `test_resulting_edit_size_reapplies_crlf_to_new_string_newlines` pins that `new_string`'s `\n` bytes are correctly re-measured as `\r\n` on a CRLF file, not counted literally (which would undercount by one byte per inserted line); `test_resulting_edit_size_preserves_crlf_bytes` covers the simpler single-line case. `test_resulting_edit_size_non_unique_without_replace_all_returns_none` pins that a non-unique `old_string` with `replace_all` unset fails open rather than estimating a misleading size, matching the real `Edit` tool's own refusal in that case. `test_resulting_edit_size_old_string_not_found_returns_none` and `test_edit_nonexistent_file_fails_open` pin the fail-open contract for the cases where the real Edit tool would independently refuse the call anyway, so a wrong size estimate here can never cause an incorrect write. `test_load_limit_bytes_non_dict_root_falls_back` and its end-to-end counterpart `test_hook_config_non_dict_root_still_blocks` pin a second `/review`-found regression: a syntactically valid but non-dict `hook-config.json` root used to raise `AttributeError` past the fail-open handler, silently disabling the guard for that call.
...
86. **skill-file-size-advisory test** — required when changing `claude/scripts/skill-file-size-advisory.py` ([ADR-127](adr/127-skill-file-size-guard.md), dev-env#939). Same two-layer shape as item 85, but every Layer-2 test writes a real on-disk fixture file rather than relying on `tool_input` content, since this `PostToolUse` hook always `os.path.getsize()`s the file *after* the write/edit already landed — there is no "resulting size" to estimate.
    ...
    Notable pins: the warn threshold is inclusive (`>=`) — `test_file_exactly_at_warn_threshold_advises` and `test_file_one_byte_under_warn_threshold_silent` pin the boundary in the opposite direction from item 85's guard hook, since a watermark is a "you're here or past it" signal rather than a ceiling. `test_bash_tool_name_not_matched` pins that this hook is deliberately **not** wired to the `Bash` matcher, unlike its neighbor `journal-shard-write-advisory.py` — a `SKILL.md` read/cat via Bash must not trigger it. `test_load_bytes_config_independent_override` confirms `skill_file_size_warn_bytes` and `skill_file_size_limit_bytes` fall back to their own defaults independently, so configuring one never silently moves the other; `test_load_bytes_config_non_dict_root_falls_back` / `test_hook_config_non_dict_root_still_advises` pin the same non-dict-config-root regression fix as item 85. The dedup sentinel (`_hookutil.sentinel_path()`, keyed on `session_id` + a hash of `file_path`) is pinned by `test_dedup_second_call_same_session_same_file_silent` (a second call for the same session+file is silent), `test_dedup_different_files_same_session_both_advise` and `test_dedup_different_session_same_file_both_advise` (the key is genuinely per-session-per-file, not a blanket suppression), and `test_no_session_id_skips_dedup_always_advises` (a payload with no `session_id` can't be deduped, so it always advises rather than silently going quiet).

    ```bash
    py -3 claude/scripts/tests/test_skill_file_size_advisory.py
    ```

87. **`_skill_file_size` shared-module test** — required when changing `claude/scripts/_skill_file_size.py` ([ADR-127](adr/127-skill-file-size-guard.md) Addendum, dev-env#939). Direct pure-function coverage of `is_skill_md()` and `load_config()` — the basename match and `.claude/hook-config.json` loader shared by both size-guard hooks, extracted from what was previously near-duplicated logic in each hook (held in sync only by a prose comment). `test_load_config_non_dict_root_falls_back` iterates a list, string, and number as config roots, confirming each falls back to defaults rather than raising `AttributeError`. Both hooks' own test files (items 85/86) also exercise this module indirectly through their thin wrapper functions (`_is_skill_md`, `load_limit_bytes`, `load_bytes_config`).

    ```bash
    py -3 claude/scripts/tests/test_skill_file_size.py
    ```

88. **journal-shell-write-guard test** — required when changing `claude/scripts/pre-tool-use-journal-shell-write-guard.py` ([ADR-129](adr/129-journal-shell-write-guard.md) + Amendment 1, dev-env#961, dev-env#962). Two layers: pure-function tests against the imported module (`journal_path_kind`, `find_bash_redirect_targets`, `find_tee_targets`, `find_serializer_journal_mentions`, `find_powershell_write_targets`, `find_journal_shell_writes`, `_is_overridden`, `_might_write_journal_content`), plus subprocess end-to-end tests driving the real hook over stdin. 63 cases as of Amendment 1 (grown from the original 39 after `/review` on PR #962 found 16 findings, all fixed and pinned in the same PR).

    Notable pins, original design: `test_find_bash_redirect_targets_heredoc_declaration_line` pins the `_mask_first_line_quotes` `<<`-neutralization fix for the classic `cat <<'EOF' > <journal-file>` shape. `test_find_journal_shell_writes_heredoc_body_mention_not_triggered`, `_git_add_commit_push_not_triggered`, `_rm_remove_item_not_triggered`, `_plain_reads_not_triggered`, and `_mkdir_new_item_directory_not_triggered` together pin every documented-legal operation on these paths as explicitly out of scope.

    Notable pins, Amendment 1 (each pins a `/review` finding fixed post-merge-checkpoint, before merge): `test_find_bash_redirect_targets_escaped_apostrophe_in_prose` is now the single most important test in the suite — an unquoted `\'` (the standard Bash apostrophe-in-prose workaround) silently defeated detection until `_neutralize_unquoted_escaped_quotes` was added; its sibling `_trailing_backslash_before_close_quote_not_regressed` pins that this fix does NOT break a single-quoted string legitimately ending in a literal backslash (a blind, context-free version of the fix regressed exactly this case, caught only by re-testing pre-fix behavior directly). `test_find_powershell_write_targets_sub_expression_read_not_a_target` and `_quoted_log_message_not_a_target` pin that only the cmdlet's actual bound path argument counts as a target, not any journal-shaped mention anywhere on the line. `test_find_journal_shell_writes_powershell_cmdlet_gated_on_tool_name` pins both the `tool_name`-gating and segment-start-anchoring fixes for a Bash command merely containing a cmdlet-shaped word. `test_find_powershell_write_targets_backslash_windows_path` pins that `_tokenize_line`'s `posix=False` switch preserves Windows backslash paths. `test_find_journal_shell_writes_node_e_continuation_line_write` and `_py_c_continuation_line_write` pin detection of the exact dev-env#904 incident shape (the write target on a later physical line than the interpreter invocation), previously undetected entirely. `test_find_journal_shell_writes_cross_repo_false_positive_not_triggered`, `_sessions_component_self_sufficient`, and `_relative_path_needs_journal_cwd` pin `_target_is_genuinely_journal`'s sessions-or-cwd gate against an unrelated repo's same-shaped file. `test_main_powershell_env_override_bypasses_block` and `test_is_overridden_powershell_env_assignment_applies_forward` pin the new `$env:ALLOW_JOURNAL_SHELL_WRITE=1` PowerShell override form; `test_main_override_does_not_exempt_unrelated_later_segment` pins that a Bash override no longer exempts an unrelated later segment. `test_find_journal_shell_writes_multiline_quoted_argument_is_a_known_accepted_gap` pins a deliberately-unfixed residual gap (a redirect hidden by a preceding multi-line quoted argument) as an explicit, asserted decision rather than an unexamined one. `test_might_write_journal_content_true_for_every_marker`/`_false_for_ordinary_commands` pin the cheap pre-filter added for performance (a first, regex-based attempt measured SLOWER than the walk it was meant to replace; the shipped plain-`in`-check version measured a genuine ~24x speedup — see the ADR amendment for the full benchmark).
...
90. **`_gh_issue_state` shared-module test** — required when changing
    `claude/scripts/_gh_issue_state.py` (dev-env#967, [ADR-131](adr/131-retro-chain-idempotent-refill.md)).
    Direct pure-function coverage of the generic GitHub issue/PR REST state helpers extracted from
    `claude/scripts/reconcile-pending-tiles.py` (a behavior-preserving move, not a rewrite — item
    80's already-hardened logic is reused rather than re-derived): `repo_from_issue_url` (strict
    `https://github.com/<owner>/<repo>` validation, including the same foreign-host and dot-segment
    rejection cases item 80 pins), `issue_states_from_rows` (the REST-conflates-PRs-with-issues
    filter and the lowercase-`state` normalization — both already-hard-won fixes from #882),
    `should_stop_paging`, and `is_closed` (case-sensitive, `"CLOSED"`-only — the paired regression
    pin that catches the `issue_states_from_rows` upper-casing being dropped, since `None`/`"OPEN"`/
    an unrecognized state all read the same, silently-inert `False`), plus the associated
    regex/constants. Also pins the one function new to this module, `issue_number_from_url(url) ->
    int | None` (parses the trailing `/issues/<N>` or `/pull/<N>` segment — needed because, unlike a
    tile shard's own filename-derived identity, a *referenced* queue issue has no independent
    filename to source its number from): both path shapes, and a non-issue/PR URL returning `None`.
    `fetch_repo_issue_states` and `check_issue_state` both stay untested — both are subprocess
    boundaries (`gh api` calls), matching the existing fixture-only convention (item 80).

    `test_reconcile_pending_tiles.py` (item 80) carries the paired identity-pin assertion
    (`test_gh_issue_state_names_are_the_shared_helpers`, checking `repo_from_issue_url`,
    `issue_states_from_rows`, `should_stop_paging`, `fetch_repo_issue_states`, `check_issue_state`,
    and `_ISSUE_PROJECTION` all still resolve to this module's objects), mirroring item 21's
    `project_dirs is _journal_shards.project_dirs` pin — so a future accidental re-duplication of
    these functions back into `reconcile-pending-tiles.py` (the exact two-divergent-copies shape
    ADR-057 already exists for) fails loudly rather than silently drifting.
