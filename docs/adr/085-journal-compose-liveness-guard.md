# ADR-085: Pre-Compose Liveness Guard Against a Still-Active Journal-Writing Session
...
- **Testing.** New pure-helper test `claude/scripts/tests/test_check_journal_compose_liveness.py`
  (dev-env `## Testing` item 42) exercises `has_uncommitted_target_date_changes()` and
