...
**Review hardening.** `/review` on the PR that introduced this amendment found two ship-blocking correctness bugs and a reliability hang risk in the design above as first implemented, all fixed before merge:

1. **Sanitized-vs-raw branch name mismatch (blocking correctness).** `cmd_snapshot` stored the *sanitized* branch name (`tr '/' '-'`, needed for the filename) in the envelope's `branch` field — but `cmd_gc`'s existence checks compare that field against real git refs, which never contain the sanitized form. Since this repo's own branch-naming convention (`feat/`, `fix/`, `config/`, `chore/`, `draft/`) always contains a slash, `gc` would delete the just-written baseline on nearly every real invocation, completely silently (`cmd_gc || true`). Every fixture branch name used elsewhere in the test file is slash-free, so nothing caught this before an independent review pass traced the envelope-write and gc-read sides against each other. Fixed by adding a `branch_name_raw()` and storing *that* in the envelope; the sanitized form is now used only for the filename, which is the one place a literal `/` is actually unsafe.
2. **Imprecise remote ref matching (blocking correctness, lower severity).** `branch_exists_remotely` passed a bare branch name to `git ls-remote --heads origin`, which git matches as a `*/<name>` suffix — a sibling branch merely ending in `/<name>` (e.g. `topic/foo` when checking `foo`) falsely reported "present." This erred toward over-retention (no data loss) but deviated from the documented "removed only when confirmed gone" contract. Fixed by extracting just the ref-name column from the output and requiring an exact match against `refs/heads/<branch>`.
3. **Unbounded remote check (reliability — critical-path hang risk).** The `git ls-remote` call had no timeout and did nothing to suppress interactive credential prompts. Since `gc` now runs on `cmd_snapshot`'s hot path (every `new-branch` in an opted-in repo), an unreachable `origin` or a stale credential cache could hang branch creation indefinitely — this repo's own git credential helper GUI is documented elsewhere (see the Git Workflow Runbooks) to hang non-interactively rather than fail. Fixed with `GIT_TERMINAL_PROMPT=0 GCM_INTERACTIVE=never timeout 10` wrapping the call, so an unreachable/prompting origin now fails into the existing "check failed → keep" path within a bounded time instead of blocking indefinitely.

Two further, Windows-specific defects surfaced while extending the test suite to cover the above (not `/review` findings — caught by the tests themselves failing unexpectedly): the performance-motivated batching of `read_baseline_meta` into a single `read_baseline_meta_batch` node invocation initially piped bash-side paths to `node` over stdin, and Git Bash's MSYS2 layer does not auto-translate a POSIX-style path handed to a native Windows binary that way (unlike an argv or environment variable) — `node.exe` silently misread every path, including well-formed ones, and reported them all as unparseable. Fixed by normalizing every path through `cygpath -m` (one extra subprocess call for the whole batch, not per file) before the pipe.

See `CLAUDE.md` → Testing, item 72 for the specific regression tests each fix added (`test_kept_branch_with_slash_in_name`, `test_snapshot_with_slashed_branch_name_survives_its_own_gc`, `test_branch_exists_remotely_suffix_sibling_not_false_positive`, `test_branch_exists_remotely_prefix_sibling_not_false_positive`).
