...
**Negative:**
- The snapshot run adds one full-suite execution at branch creation. In lifting-logbook (sub-minute per Explore agent's findings) the cost is negligible; in larger repos it would dominate `new-branch` latency and the opt-in flag should stay off.
