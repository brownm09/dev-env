...
> **Close a session that involved substantial hands-on correction with a feedback-retro table.** When a session included a meaningful amount of corrective or judgment-call feedback from the user — not just task requests, but "no, not that," register/diction catches, record-reconciliation calls, process-preference corrections — close with a table: **What was said** (paraphrased) / **Underlying pattern** / **Status** (already durable, partially addressed, or not yet formalized) / **Long-term mechanism** (how to make this durable). This is distinct from the ADR-095 Completed/Context/Remaining recap, which reports task status, not correction patterns; produce both when both apply. Any row landing on "not formalized" is a candidate for the existing memory + issue-pairing discipline (ADR-038, ADR-046) — the table is a discovery pass, not a new persistence mechanism of its own.
