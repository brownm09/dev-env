...
### 2026-07-02 (follow-up 4) — Narrow the underscore-prefix exemption in `test_every_subprocess_using_hook_imports_winsubp` to `_winsubp.py` only ([dev-env#515](https://github.com/brownm09/dev-env/issues/515))

Follow-up 3 above fixed `_hookio.py`'s `confirm_merge_via_gh` missing its own `import _winsubp`, but noted the static guard that should have caught it statically did not, because of a scoping bug in the guard itself: `test_every_subprocess_using_hook_imports_winsubp`'s directory scan (`claude/scripts/tests/test_pyw_stdio.py`) skipped every file via `path.name.startswith("_")`, not only `_winsubp.py` (the one module that structurally cannot import itself). This blanket exemption silently excluded all 9 underscore-prefixed files in `claude/scripts/` — including `_hookio.py` and `_gh_project.py`, both of which contain real `subprocess.run` call sites — from the very guard meant to enforce `import _winsubp` on subprocess-using hooks.

**Audit:** manually checked all 9 underscore-prefixed files in `claude/scripts/` against `_SUBPROCESS_USE_RE` (`_winsubp.py`, `_hookio.py`, `_gh_project.py`, `_hookutil.py`, `_journal_shards.py`, `_worktree_liveness.py`, `_worktree_topology.py`, `_repo_scan.py`, `_worktree_canon.py`). Only `_hookio.py` and `_gh_project.py` contain a real subprocess use-site, and both already import `_winsubp` (`_hookio.py` since follow-up 3 above; `_gh_project.py` since its creation under ADR-073). The other 6 non-`_winsubp.py` files either don't use subprocess at all, or only mention the word "subprocess" in docstring prose (e.g. "no subprocess" in `_journal_shards.py`, `_worktree_liveness.py`, `_worktree_topology.py`, `_worktree_canon.py`), which `_SUBPROCESS_USE_RE` correctly does not match (it requires a literal `subprocess.run` / `.Popen` / etc. call or `from subprocess import`, not the bare word). **Conclusion: zero source-code changes were needed to any hook script — this is a test-only fix.**

**Fix:** Replace the blanket `if path.name.startswith("_"): continue` with a named predicate, `_is_winsubp_module(name: str) -> bool`, that returns `True` only for the exact filename `_winsubp.py`. The loop now calls `if _is_winsubp_module(path.name): continue`. Added inline negative-control assertions (matching this test's existing convention for its two regexes) that assert the predicate is `True` for `"_winsubp.py"` and `False` for representative non-`_winsubp` underscore names (`_hookio.py`, `_gh_project.py`, `_hookutil.py`) — so a future regression back to a blanket underscore-prefix skip fails the test immediately rather than silently reopening this gap.

**Why a named predicate rather than an inline filename comparison:** matches this repo's established convention for pure boolean-predicate helpers (`_is_windows()`, `_is_devenv_cwd()`, `_is_successful_merge_call()` elsewhere in `claude/scripts/`) and CLAUDE.md's `## Testing` philosophy of extracting logic into standalone functions that can be asserted against directly, rather than embedding the condition inline where only the end-to-end directory scan exercises it.

**Verification:** Ran `claude/scripts/tests/test_pyw_stdio.py` after the fix. `test_every_subprocess_using_hook_imports_winsubp`'s `checked` count increased by exactly 2 (now including `_hookio.py` and `_gh_project.py`, previously skipped by the blanket exemption) and the test still passes — empirically confirming the audit above found the complete set of affected files and that both already comply. `test_no_hook_spawns_python_via_py_launcher` was reviewed and requires no change: its scan loop has no underscore-prefix filter of any kind and already covers `_winsubp.py` and every other underscore-prefixed file unconditionally.
