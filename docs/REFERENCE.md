...
| `stub-push-archive-reminder.py` | `git push` to `engineering-journal` with a stub commit | Writes a sentinel file (`~/.claude/scratch/stub-pushed.flag`) and exits 0. Verifies the most-recent commit in the journal repo touches a `.stub.md` file before writing the flag. The Stop hook (`journal-stop-check.py`) consumes the sentinel and issues the archive reminder as a closing stdout message (exit 0 — not a blocking exit 2). The push-failure guard (`has_push_error`, reading output via the shared `_hookio.read_command_output`), the command-shape predicate (`is_git_push_command`, `scan_top_level`-anchored — ADR-050 Amendment 10), and the repo-reference predicate (`references_engineering_journal`, anchored to a `cd`/`git -C` directory argument rather than a CLI verb — ADR-050 Amendment 12) are unit-tested by `tests/test_stub_push_archive_reminder.py`. [ADR-050](adr/050-shared-hookio-sibling-hook-fixes.md) |
...
**All Stop hooks run in parallel — the list order below is not an execution order.** Per the same docs: *"all matching hooks run in parallel … every hook's command runs to completion before Claude Code merges the results. One hook returning `deny` doesn't stop sibling hooks from executing."* So a hook that exits 2 (only `stop-tile-enumeration-gate.py` below does) does **not** short-circuit the rest — `awake-blocker.py`'s sleep-lock release runs at every Stop regardless of the tile gate blocking, and reordering the list carries no meaning. See [ADR-088 → Stop-hook parallelism](adr/088-state-keyed-tile-enumeration-gate.md).
