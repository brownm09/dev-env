...
| `tests/check-remote-read-hygiene.sh` | `bash claude/scripts/tests/check-remote-read-hygiene.sh` | Lints for the [#602](https://github.com/brownm09/dev-env/issues/602) / [#877](https://github.com/brownm09/dev-env/issues/877) class — a `git show <ref>:<path>` paired with `2>/dev/null`, where MSYS mangling makes git's swallowed `fatal:` indistinguishable from "file absent". Scans every tracked file under `claude/`; keys on the co-occurrence, so the sanctioned `MSYS_NO_PATHCONV=1` form never trips. Hermetic; comment/blockquote mentions ignored. Exit 1 on any offender. See [ADR-120](adr/120-review-skill-absence-checks-over-api.md). |
