...
| Script | Trigger condition | What it does |
|--------|------------------|-------------|
| `pre-tool-use-nested-agent-background-guard.py` | Every `Agent`-tool call | **Smoke-test stage (observation-only, dev-env#935).** The `Agent` tool defaults to `run_in_background: true`; when a subagent spawns its own child subagent relying on that default, the child does not block the spawning turn — the parent can return before the child finishes, and the orphaned child's completion routes to `general-purpose`/`main` instead of back to the subagent that spawned it, silently stalling until a human notices and manually resumes it via `SendMessage` (career-playbook [ADR-090](https://github.com/brownm09/career-playbook/blob/main/docs/adr/090-synchronous-subagent-spawns.md), PR #749; confirmed recurring — a still-earlier 2026-05-13 precedent, and the same bug hit three more applications the day the prose-only fix shipped, before every spawn site was updated). This stage logs `tool_name`, `agent_id` (present only when the call fires inside a subagent — the nesting signal), `agent_type`, and whether `tool_input.run_in_background` was set (and its value) to `~/.claude/scratch/agent_hook_smoketest.jsonl`. Always exits 0 — cannot block or otherwise affect any tool call. Once confirmed live (interactively, and during one real unattended run), a follow-up PR replaces the body with real enforcement: block only when `agent_id` is present (nested) **and** `run_in_background` is omitted entirely — any explicit value passes through, so ordinary top-level spawns and deliberate nested-parallel fan-out are unaffected. |
...
resolves to `None` → kept and counted unresolved, never silently dropped.
