...
above) discovers every `claude/scripts/tests/test_*.py` and every bash gate
(`claude/scripts/tests/*.sh` + `claude/hooks/tests/*.sh`), runs each as a subprocess, and exits
