...
Per-session sentinel helpers and transcript-locate are extracted into `claude/scripts/_hookutil.py` (Stop / UserPromptSubmit hook family — the analogue of `_hookio.py` for the PostToolUse family). It exposes `cleanup_stale_sentinels(prefix)`, `sentinel_path(prefix, session_id) -> Path`, and `find_transcript(session_id) -> Path | None` — used by `posttooluse-inert-advisory.py`, `reconcile-open-prs.py`, and `token-tracker.py` so each no longer carries its own `SCRATCH` / `PROJECTS` constants and local copies of these helpers. All three parameters (`scratch`, `projects`) are injectable for offline testing. See [ADR-064](adr/064-shared-hookutil-sentinel-transcript-locate.md).
...
| `stub-push-archive-reminder.py` | `git push` to `engineering-journal` with a stub commit | Writes a sentinel file (`~/.claude/scratch/stub-pushed.flag`) and exits 0. Verifies the most-recent commit in the journal repo touches a `.stub.md` file before writing the flag. The Stop hook (`journal-stop-check.py`) consumes the sentinel and issues the archive reminder via exit 2. The push-failure guard (`has_push_error`, reading output via the shared `_hookio.read_command_output`) is unit-tested by `tests/test_stub_push_archive_reminder.py`. [ADR-050](adr/050-shared-hookio-sibling-hook-fixes.md) |
...
Scans all primary git repos directly under `C:/Users/brown/Git` and removes `claude/*` worktrees whose branches are fully merged into `origin/main`, and **parks any non-primary worktree squatting `main`** back onto its own `claude/<slug>` branch (recreated at HEAD via `git checkout -b` — non-destructive, frees the ref even for a dirty worktree the old `git worktree remove` refused; [ADR-058](adr/058-worktree-squatting-main-detection-correction.md)). Repos with no GitHub remote are skipped. Uses `git branch -d`, `git worktree remove` (no `--force`), and `git checkout -b` (parking). Skips the current worktree, dirty worktrees (for removal), any worktree not named `claude/*` (except `main`, which is parked off), and — since this routine runs out-of-process and cannot see other sessions via cwd — **any worktree with an active Claude session** (transcript activity within 24h; override with `--liveness-window-min`); the liveness guard runs before the park, so only an *idle* squatter is moved. Sends a push notification listing any unmerged branches that were skipped. [ADR-051](adr/051-worktree-liveness-guard.md), [ADR-058](adr/058-worktree-squatting-main-detection-correction.md)
...
Reconciles the **Dev Env** project board (#3) against the repo's open issues: lists open issues and board items, computes the set difference (orphans = open issues not on the board), adds each orphan via `gh project item-add`, then surfaces the orphans — and any pre-existing **open** board items — still missing a required field (Impact / Why), printing the exact `gh project item-edit` commands. **Add-only + report-only:** it never sets a field value (no guessing) and never mutates single-select options, so it is safe unattended. Backstop for the gap [ADR-053](adr/053-posttooluse-hooks-inert-in-background-sessions.md) documents — `post-tool-use.py` can't board issues filed in background/`spawn_task`/SDK sessions. Reads `.claude/hook-config.json` (the same config `post-tool-use.py` uses, so the field IDs never drift). Runs `sync-routine-worktree` as Step 0; push-notifies when issues still need Impact/Why. The engine is the on-demand `reconcile-project-board.py` (Utilities below). [ADR-068](adr/068-reconcile-project-board-orphan-issues.md)
...
| `prune-merged-worktrees.py` | `py -3 prune-merged-worktrees.py [--dry-run] [--repo-path /path/to/repo\|--scan-dir /path/to/dir] [--liveness-window-min N]` | Manual equivalent of the prune routines. Auto-detects the GitHub repo slug from the origin remote URL. `--repo-path` targets a specific repo's worktrees (defaults to dev-env); `--scan-dir` discovers and prunes all git repos directly under the given directory. Removes merged `claude/*` worktrees and parks any worktree squatting `main` off onto its own branch ([ADR-058](adr/058-worktree-squatting-main-detection-correction.md)). Skips any worktree with an active Claude session (transcript activity within `--liveness-window-min`, default 1440 = 24h). [ADR-051](adr/051-worktree-liveness-guard.md), [ADR-058](adr/058-worktree-squatting-main-detection-correction.md) |
...
| `reconcile-project-board.py` | `py -3 reconcile-project-board.py [--repo-root PATH] [--dry-run]` | Engine behind the `reconcile-project-board` routine (and an on-demand board check). Reads `.claude/hook-config.json`, lists open issues + project items, computes orphans (open issues not on the board), adds each via `gh project item-add`, then reports the orphans + any pre-existing **open** board items still missing a required field, printing the exact `gh project item-edit` commands. **Add-only + report-only** — never sets a field value (no guessing) and never mutates single-select options. `--dry-run` reports without adding; the repo root defaults to the canonical checkout (so it works from a worktree). Detects a missing `project` scope and prints the `gh auth refresh -s project` hint (exit 1). [ADR-068](adr/068-reconcile-project-board-orphan-issues.md) |
