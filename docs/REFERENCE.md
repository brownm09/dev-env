...
| Script | What it does |
|--------|-------------|
| `session-mode-prompt.py` | Fires on the first user prompt of each new session. Exits 2 to block the prompt and shows a mode-selection reminder (plan / bypass / auto). Uses a 2-minute marker file in `scratch/` so the re-submit after the user confirms/adjusts the mode passes through. Addresses sessions that inherit the foreground session's bypass-permissions mode instead of reading `defaultMode` from `settings.json`. Suppressed for automated sessions whose prompt begins with an XML tag (e.g. `<scheduled-task>`, `<ci-monitor-event>`); human prompts never start with an XML tag. |
