...
| Script | What it does |
|--------|-------------|
| `session-mode-prompt.py` | Fires on the first user prompt of each new session (no prior assistant turns). Exits 2 to block the prompt and shows a mode-selection reminder (plan / bypass / auto). Uses a 2-minute marker file in `scratch/` so the re-submit after the user confirms/adjusts the mode passes through. Addresses sessions that inherit the foreground session's bypass-permissions mode instead of reading `defaultMode` from `settings.json`. Suppressed in Claude-managed worktree sessions (`.claude/worktrees/` in cwd). |
