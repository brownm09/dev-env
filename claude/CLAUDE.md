...
- **Write a stub on PR merge.** A merge is a session boundary that requires stub coverage.
  ...
  - **Same session as PR open:** the existing stub covers the full lifecycle. Before committing the merge record, update the stub's session block with any significant work that occurred after the PR was opened (review findings, investigation, rule revisions, etc.). Then append a second manifest line with `prs_closed: [N]`, remove the PR from `open-prs.jsonl`, commit, and stop.
