...
- **Merging a PR developed in a worktree:** Call `ExitWorktree` with `action: "remove"` before running `gh pr merge`. `action: "remove"` deletes both the worktree directory and the local branch, so `gh pr merge --squash --delete-branch` only has to delete the remote branch (steps 1–2 of `gh`'s sequence) — the local step is already done. `action: "keep"` does NOT help: it only changes the working directory; the worktree remains registered and `git branch -d` still fails. If `ExitWorktree` returns a no-op ("No-op: there is no active EnterWorktree session to exit" — worktree was created in a previous session), manually remove it from the main repo before merging: `git -C C:/Users/brown/Git/dev-env worktree remove .claude/worktrees/<name> && git -C C:/Users/brown/Git/dev-env branch -D <branch>`, then proceed with `gh pr merge --squash --delete-branch` as normal. If you see the "cannot delete branch checked out at …" error, the merge and remote branch delete already completed — only the local branch remains and will be collected by the weekly `prune-merged-worktrees.py` run.
