...
- **Claude Code has no user-scope `settings.local.json` — only a per-project one; project-scoped app-written content belongs there.** Verified against the [Claude Code settings documentation](https://code.claude.com/docs/en/settings) and the shipped binary: the four settings files are User (`~/.claude/settings.json`, every project on this machine), Shared project (`.claude/settings.json`), **Project local** (`.claude/settings.local.json`, "you, in this one project only"), and Managed. There is no fifth, user-scope local file — a `~/.claude/settings.local.json` would simply never be read. So when the Claude Code app writes a key that embeds **project-specific** content (an auto-mode environment scan, a per-project credential hint — anything naming a specific repo's paths, visibility, or policies), relocate it to *that project's own* `.claude/settings.local.json` rather than leaving it in the user-scope file: the user file loads for **every** project's session on this machine, so project-scoped state there is both wrong in scope and needlessly broad in what it exposes to unrelated sessions. Project-local is the one real mechanism for personal, project-scoped, git-excluded state (confirm the project's `.gitignore` actually excludes it — `git check-ignore -v .claude/settings.local.json` — since Claude Code only auto-excludes a file *it* created). An ordinary global preference toggle carrying no project-specific content (a boolean like `theme`/`tui`/`*NotifEnabled`) is unaffected and stays in the user file. Incident: a career-playbook `autoMode.environment` scan — a written description of the user's personal career-search data — sat in the user-scope file, loading for every project ([dev-env#1049](https://github.com/brownm09/dev-env/issues/1049), [dev-env#1050](https://github.com/brownm09/dev-env/issues/1050)). See [ADR-139](../docs/adr/139-machine-local-settings-with-shared-source-sync.md).
...
- **A PR body, issue body, or commit message piped into a command's stdin is content too.** `gh pr create --body-file - <<'EOF'`, `gh issue comment --body-file -`, `git commit -F - <<'EOF'` — same prose, same shell, and *no file left behind to inspect* when it corrupts. Write the body to a file, then pass that path: `--body-file "$TMPFILE"`, `git commit -F "$TMPFILE"`. This does **not** extend to a heredoc feeding an *interpreter's* stdin (`py -3 - <<'PY'`, `python - <<'EOF'`) — that body is a program, not file content, and stays entirely fine. ([ADR-138](../docs/adr/138-shell-content-write-guard.md) Amendment 1, on 54,330 measured commands.)
...
Mechanically enforced by `pre-tool-use-shell-content-write-guard.py`, which blocks a Bash/PowerShell command that routes an inline literal to a file — or, since ADR-138 Amendment 1, into a `gh`/`git` prose argument (`--body-file -`, `--notes-file -`, `-F -`) — when that literal is multi-line or carries an apostrophe, backtick, or backslash. Program-output redirection and an interpreter's stdin are never matched — structurally, not heuristically. Genuine exception: prefix `ALLOW_SHELL_CONTENT_WRITE=1` (Bash) or precede with `$env:ALLOW_SHELL_CONTENT_WRITE=1` (PowerShell). Replaying the guard over 54,330 real commands measured its block set at **~12× the baseline rate of real shell-parse failures** — it concentrates the harm rather than over-matching; re-check with `py -3 claude/scripts/replay-shell-content-guard.py --gap`. See [ADR-138](../docs/adr/138-shell-content-write-guard.md).
...
5. **An absence claim needs an absolute path, the right ref — and, after a write, a fresh read.** Before concluding a file, pattern, or config is *not present*, re-run the check rooted at the repo root (an absolute path, or `git -C <root>`) and let its stderr through. `session-start-sync.py` now fetches automatically at session start ([ADR-130](../docs/adr/130-session-start-fetch-ff-only-or-warn.md)), so `origin/main` itself is more often fresh going into an investigation — but it fires once per session, not before each individual absence claim, so the five mechanisms below (and this whole rule) still apply mid-session. Five independent mechanisms each turn a partial or stale view into output indistinguishable from a genuine miss:
   ...
   - **Ref scoping.** The same miss happens on the *ref* axis, where an absolute path does not help: a working tree, `HEAD`, and `origin/main` each answer only for themselves. Before claiming a name/number/identifier is free — an ADR number, a `## Testing` item, a migration or fixture filename — check all **three** surfaces, because they are three different points in a branch's lifecycle and none of them sees the others: `origin/main` (**merged**), the open-PR set (`gh pr list`, `gh pr diff <N> --name-only`) (**PR'd**), and the remote branches (`git fetch origin`, then `git for-each-ref --sort=-committerdate refs/remotes/origin` and `git ls-tree -r <ref> --name-only -- <dir>/` on the recent ones) (**pushed, no PR yet**). Filter that third surface by recency, not ancestry — in a squash-merge repo `git merge-base --is-ancestor` calls every *merged* branch unmerged, so it separates nothing. `ls-tree` answers only for identifiers that are *filenames*; for one embedded in a file's **content** (a `## Testing` item number) read the blob — `gh api "repos/{owner}/{repo}/contents/<path>?ref=<ref>" -H "Accept: application/vnd.github.raw"`, never `git show <ref>:<path>`, which mangles per *Suppressed failure* below. The third surface is the one usually skipped and the one that matters most: **pushed-but-not-yet-PR'd is exactly the window two concurrent sessions collide in**, and it is invisible to the other two. Incidents: `ls docs/adr/` and `INDEX.md` both showed 115 as the highest ADR, so 116 looked free — open PR [dev-env#863](https://github.com/brownm09/dev-env/pull/863) had already claimed it; and on 2026-08-28 two sessions each claimed ADR-143, **both having run the checkout-plus-open-PR check correctly**, because each other's branch was pushed without a PR — caught only by the luck of a shared scratch filename ([dev-env#1080](https://github.com/brownm09/dev-env/issues/1080)).
   ...
   - **Suppressed failure.** Never pair an absence check with `2>/dev/null` — the failure you most need to see is the one it hides. MSYS path-conversion mangles `git show <ref>:<path>` into `origin\main;.github\…`; git exits non-zero, the redirect swallows the `fatal:`, and the empty output reads exactly like "the pattern is not present" ([dev-env#602](https://github.com/brownm09/dev-env/issues/602)). This is **deterministic, not intermittent** — the trigger is a leading-dot path segment immediately after the `:`, independent of path depth (`origin/main:.gitignore` mangles; `origin/main:claude/skills/review/SKILL.md` does not); full detail in the `/review` skill's [Remote reads on Windows](skills/review/SKILL.md) note. Prefix `MSYS_NO_PATHCONV=1`, or read the blob over the API: `gh api "repos/{owner}/{repo}/contents/<path>?ref=<ref>" -H "Accept: application/vnd.github.raw"`.
   - **Read-after-write staleness.** A *filtered list* read is eventually consistent, so a query issued moments after a write can under-report with a clean `200` — a short list indistinguishable from "the write did not land." Never take one as evidence a write failed: re-read after a delay, or confirm against the single-object endpoint (`gh api "repos/{owner}/{repo}/issues/{n}"`), which returned correct state immediately in the observed incident — GitHub publishes no read-your-writes guarantee for it, so treat the delayed re-read as the reliable check and the direct read as the fast one. This belongs to the same false-absent family as the pipe-decode/raw-bytes trap (item 6) and the `$`-anchored-CRLF trap under Git Workflow, but is **alone in it: the obvious diagnosis is wrong *and* self-confirming** — a retry with an unrelated flag bolted on (`--paginate`) returns the full set, so the flag looks causal when the variable that actually changed was elapsed time. A fix that passes its own retry test is the kind that sticks once written down, so confirm the mechanism, not just that the retry went green. Incident: [dev-env#1047](https://github.com/brownm09/dev-env/issues/1047) — six `POST .../issues/{n}/labels` calls each echoed the label back, then `gh api "repos/…/issues?labels=start-here&state=open&per_page=100"` returned five; the omitted issue read correctly from its own endpoint, and minutes later the identical query returned all six.
...
6. **A locale-decoded pipe both misreports corruption and creates it — check raw bytes before believing either.** Piping matched or filtered text through an intermediate interpreter's stdin — e.g. `rg`'s matches relayed into `py -3 -c "..."` — or reading a subprocess with `text=True` and no `encoding=`, introduces a decode step whose codepage need not match the source encoding: on this Windows/Git-Bash setup `locale.getpreferredencoding(False)` is **cp1252**, so correctly UTF-8-encoded bytes are silently reinterpreted — an em dash (U+2014, UTF-8 `E2 80 94`) becomes the three codepoints U+00E2, U+20AC, U+201D, the textbook signature of UTF-8 misread as windows-1252. That decode has **two outcomes, and only the loud one is already handled**: a byte cp1252 leaves *undefined* (`0x81`, `0x8D`, `0x8F`, `0x90`, `0x9D`) raises `UnicodeDecodeError` ([dev-env#503](https://github.com/brownm09/dev-env/issues/503)), while a byte cp1252 *can* map produces wrong characters and no error at all. `_winsubp.py` forces UTF-8 and closes both for dev-env's own committed hooks ([ADR-007](../docs/adr/007-hook-command-invocation.md)); an ad-hoc script written inline in a session imports nothing and is protected from neither. Two directions, reaching opposite conclusions:
   - **Read direction — the pipe misreports.** A genuinely clean file relayed through such a decode reads exactly like real corruption. Before concluding the file itself is corrupted, verify against its raw bytes directly — `open(path, 'rb')` in Python, or equivalent — bypassing the pipe entirely; a pipe-relayed "corruption" that a direct byte read doesn't reproduce is a diagnostic-pipeline artifact, not a real defect. Incident: [dev-env#952](https://github.com/brownm09/dev-env/issues/952), surfaced mid-investigation of [career-playbook#1139](https://github.com/brownm09/career-playbook/issues/1139).
   - **Write direction — the pipe creates the corruption and you ship it.** **Never round-trip remote content through a locale-decoded pipe.** A read-modify-write of any GitHub body (issue, PR, comment) whose read is `subprocess.run([...], capture_output=True, text=True)` writes the *mangled* text back to the live resource — both `gh` calls return `200`, and nothing surfaces an error. Read bytes and decode explicitly: `subprocess.run(..., capture_output=True)` **without** `text=True`, then `.decode("utf-8")` — or redirect to a file and read it with `encoding="utf-8"`. **This direction inverts the read direction's conclusion**, so a session carrying only "mojibake through a pipe is probably an artifact" is primed to dismiss corruption it just caused. Settle it **numerically, never visually** — a console-printed sample is re-encoded on its way to the terminal, so it cannot distinguish corrupt *data* from a corrupt *display*; count codepoints instead: `collections.Counter(ord(ch) for ch in body if ord(ch) > 127)`. The transform is deterministic, so recovery is `corrupted.encode("cp1252").decode("utf-8")` — lossless only while every mangled codepoint still round-trips through cp1252; a U+FFFD left by an `errors="replace"` decode has already destroyed the original, and note that `errors="replace"` is exactly what `_winsubp` installs. Incident: [dev-env#1073](https://github.com/brownm09/dev-env/issues/1073) — 33 × U+2014 in [#963](https://github.com/brownm09/dev-env/issues/963)'s body written back as 38 × U+00E2, 35 × U+20AC, 33 × U+201D; see [ADR-143](../docs/adr/143-locale-decoded-pipe-writes-corruption.md).
...
- **Check for an existing issue before filing a new one.** Before `gh issue create` for a bug report or defect, search both open and closed issues for the same problem: `gh issue list --search "<keywords>" --state all` (add `--repo <owner>/<repo>` when the defect lives in a different repo than the session's cwd — see *CLI Scripting Checklist* for why a cwd/target mismatch here silently searches the wrong repo's issues). A match against an **open** issue means it's already tracked — reference it instead of filing a duplicate, adding a comment with any new detail this occurrence surfaces. A match against a **closed** issue means verify it was actually fixed — `gh issue view <N> --json state,closedAt` plus the linked closing PR's diff — before treating a new report as valid; if the old fix didn't stick or missed a case, reopen it or reference it rather than filing fresh. This is the filing-time sibling of two other checks, not a repeat of either: the memory-immortalization issue in *Durable Preferences & Memory → Search before filing* is a specific case of this same rule; *Check for an existing open PR before implementing* (next bullet) is the same discipline one stage later — after an issue exists, before implementing it. Incident: career-playbook [#1575](https://github.com/brownm09/career-playbook/issues/1575) (filed 2026-09-08) and [#1608](https://github.com/brownm09/career-playbook/issues/1608) (filed 2026-09-14) independently diagnosed the identical `scripts/lib/changed.sh` test-override-seam bug from scratch, six days apart, each while running the pre-PR `validate.sh` gate for an unrelated PR — fixed together in career-playbook PR [#1610](https://github.com/brownm09/career-playbook/pull/1610); the overlap surfaced only by chance, via an unrelated session's journal note. See [ADR-048](../docs/adr/048-memory-immortalization-issue-pairing.md) (Amendment, 2026-09-15).
- **Check for an existing open PR before implementing.** Before starting implementation work on any issue — freshly filed or picked up from the backlog — confirm no open PR already addresses it: `gh pr list --search "<issue-number-or-keywords>" --state open`, or `gh issue view <N> --json` and check its linked-PRs/timeline. A match means the work is already in flight — **surface it to the user via `AskUserQuestion`** (closing, superseding, or consolidating onto an existing PR is a visible action, not one to resolve unilaterally) rather than silently duplicating the effort. This is the mirror image of the preceding bullet, not a repeat of it: *Check for an existing issue before filing a new one* confirms no duplicate issue exists before one is filed at all; this bullet confirms no fix is already in flight under an open PR once an issue exists, before implementing it. Distinct too from *CLI Scripting Checklist → Ref scoping*, which confirms all three ref surfaces (`origin/main`, open PRs, and remote branches) before claiming an identifier (an ADR number, a filename) is *free*. None of the three covers starting work on an issue that already has a fix in flight under an open PR — possibly filed against a different, older duplicate issue. Incident: a session fixing dev-env#918 discovered, only while writing the journal stub, that PR [dev-env#770](https://github.com/brownm09/dev-env/pull/770) (open since 2026-07-14) already fixed the identical bug via the older duplicate issue [dev-env#734](https://github.com/brownm09/dev-env/issues/734). See [ADR-125](../docs/adr/125-check-open-pr-before-implementing-issue.md).
...
- **ADR-warrant check** at three checkpoints — (1) after a plan is approved / first edit, (2) after `gh pr create`, (3) before `gh pr merge`. Warranted when the change touches a rule/hook/skill/settings documented in `claude/`, restructures a `claude/` directory, sets a workflow rule other CLAUDE.md files reference, or has rationale `git log` won't recover. Scan `docs/adr/INDEX.md` tags first; open ADR files only on a tag match. Global rules → dev-env [`docs/adr/`](../docs/adr/INDEX.md); project decisions → that project's `docs/adr/`. **Write the ADR + update `INDEX.md` before merging a qualifying change.** For clearly ADR-worthy multi-file work, create the template at checkpoint 1 ([ADR-011](../docs/adr/011-adr-warrant-check.md)). **Once warranted, prefer the smallest durable home.** The four criteria above are *trigger* conditions — they answer whether the rationale must be written down, not *where* it goes, and defaulting every trigger to a new file is how one day produced five ADRs (2026-07-22). A **new ADR** is for a decision that could later be reversed on its own; an **amendment** is for anything that refines, corrects, extends, or bounds a decision already recorded — if the honest opening sentence is "as ADR-N says, except…", it is an amendment, and a follow-up fix to machinery an ADR introduced is *always* one; a **plain CLAUDE.md line with no ADR** is for a rule whose reasoning is fully contained in the rule as written. Tie-break: **if you cannot name an alternative you rejected, it is not a new ADR** — every ADR carries an *Alternatives Considered* section, so this is checkable in seconds ([ADR-145](../docs/adr/145-scope-discipline-self-referential-churn.md) §1).
...
Motivating incident: [lifting-logbook PR #604](https://github.com/merickvaughn/lifting-logbook/pull/604).
...
Confirmed as a **general git-worktree mechanic**, not a quirk of any one repo's worktree count: [dev-env#575](https://github.com/brownm09/dev-env/pull/575) (canonical correctly on `main`) and [lifting-logbook#664](https://github.com/merickvaughn/lifting-logbook/pull/664) (an idle worktree left squatting `main` by an earlier merge; that repo's CLAUDE.md "Standard Issue Workflow" step 8 carries its own copy of this same two-step pattern). Full detection, root cause, and non-destructive auto-correction (parking idle squatters instead of removing them): dev-env [ADR-058](../docs/adr/058-worktree-squatting-main-detection-correction.md); complete runbook: dev-env [`docs/REFERENCE.md` → Git Workflow Runbooks](../docs/REFERENCE.md#git-workflow-runbooks).
...
**Scope boundary — this section governs *experiments*; standing *gates* are Pass 3's Gate-calibration dimension.** The calibration law is one law with two application sites: an instrument calibrated against known-good **and** known-bad references before it bears weight, and an uncalibrated check demoted to a diagnostic. What is experiment-only, and must not be imported into a gate review, is everything built around *comparing arms to reach a verdict* — tiers, the pre-registration freeze, the incumbent-influence inventory, blinding and order-randomization, the win bar, the T1–T10 sweep, and the verdict/decision vocabulary. A gate has no arms, no baseline, and no verdict; it has a threshold, and what it owes is the provenance of that threshold ([ADR-144](../docs/adr/144-gate-calibration-pass-3-dimension.md)).
...
**Pass 3 — Risk-dimension audit:** before acting, the plan must address each of these eight dimensions explicitly. For any that don't apply, state **`N/A — <reason>`** rather than omitting it. The bar is *stating the decision*, not adding work everywhere.
...
8. **Gate calibration** — when the plan introduces or tightens an automated check that classifies inputs the plan does not enumerate (a hook, CI guard, validator, linter rule, LLM judge, or any threshold/ratio/count applied to a population), state: the **measured property** it keys on, not the shape the author expects to see; the **calibration corpus** — known-good *and* known-bad references, with counts and the check's measured hit rate on each (the known-bad reference is also what proves the check does not pass *vacuously*); **where each constant came from** — the corpus member *and its class*, since a bare "calibrated: yes" hides a threshold read off the wrong class; and the **margin** to the worst known-good observation, *that observation named* — naming it is what makes a baseline sourced from the wrong class collide visibly with the corpus listing, where a bare ratio would not. **An uncalibrated check is a diagnostic: it may report, but it may not block, fail, or take a destructive action.** Same calibration law as `## Experimental Rigor`, applied to standing gates rather than experiments — worksheet form is `/experiment-audit` Step D4, references live in the project `## Experiments` registry where one exists, and a gate carries none of the tier / pre-registration / blinding / verdict machinery. A fixture-pinned unit test is out of scope: it enumerates its own inputs. **A check whose input is *extracted* — a section scan, a regex, a glob, a parse that degrades to `[]` rather than raising — additionally owes an assertion that the extraction is non-empty, separate from the comparison itself**, because a comparison against nothing succeeds and reports a confident passing count; ADR-116's own gate is the worked example (`test_runbook_section_exists` / `test_runbook_has_runnable_lines`, whose failure message reads *"a vacuous pass, not a clean one"*), and running this dimension backwards over the existing inventory found the two live instances in [ADR-145](../docs/adr/145-scope-discipline-self-referential-churn.md) §3. Plans introducing or tightening no such check: `N/A — no gate introduced or tightened` (see [ADR-144](../docs/adr/144-gate-calibration-pass-3-dimension.md)).
...
**Rationale incident.** lifting-logbook [PR #395](https://github.com/merickvaughn/lifting-logbook/pull/395) (2026-06-01). The `staging.yml` deploy-prereq guard printed a "Staging Clerk secret key is not configured" error and pointed to `docs/deploy.md` Step 4. The actual upstream chain was a transient 504 from Artifact Registry in `build-images` → `deploy-api` skipped because of `needs: build-images` → empty `clerk_configured` output → integration-tests' `!= 'true'` check fired the misconfig message. The secret was already correctly set. Both the github-actions bot and I reported the wrong root cause until the user pushed back. Full trace: [PR #395 comment](https://github.com/merickvaughn/lifting-logbook/pull/395#issuecomment-4594434736). See [ADR-034](../docs/adr/034-error-message-diligence.md).
