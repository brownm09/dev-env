...
6. **A locale-decoded pipe both misreports corruption and creates it — check raw bytes before believing either.** Piping matched or filtered text through an intermediate interpreter's stdin — e.g. `rg`'s matches relayed into `py -3 -c "..."` — or reading a subprocess with `text=True` and no `encoding=`, introduces a decode step whose codepage need not match the source encoding: on this Windows/Git-Bash setup `locale.getpreferredencoding(False)` is **cp1252**, so correctly UTF-8-encoded bytes are silently reinterpreted — an em dash (U+2014, UTF-8 `E2 80 94`) becomes the three codepoints U+00E2, U+20AC, U+201D, the textbook signature of UTF-8 misread as windows-1252. That decode has **two outcomes, and only the loud one is widely known**: a byte cp1252 leaves *undefined* (`0x81`, `0x8D`, `0x8F`, `0x90`, `0x9D`) raises `UnicodeDecodeError` ([dev-env#503](https://github.com/brownm09/dev-env/issues/503)), while a byte cp1252 *can* map produces wrong characters and no error at all. `_winsubp.py` forces UTF-8 and closes both for dev-env's own committed hooks ([ADR-007](../docs/adr/007-hook-command-invocation.md)); an ad-hoc script written inline in a session imports nothing and is protected from neither. Two directions, reaching opposite conclusions:
