...
- **Write a stub on PR merge.** A merge is a session boundary that requires stub coverage.
  - **Same session as PR open:** the existing stub covers the full lifecycle. Update its manifest entry to set `prs_closed: [N]` and remove the PR from `open-prs.jsonl`. Then stop.
