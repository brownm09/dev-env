...
- **Write a stub on PR merge.** A merge is a session boundary that requires stub coverage.
  ...
  - **Same session as PR open:** the existing stub covers the full lifecycle. Before the final commit below, update the stub's session block with any decisions made or discoveries surfaced after the PR was opened (new dependencies, design pivots, review findings that changed the implementation, etc.). Then append a second manifest line with `prs_closed: [N]`, remove the PR from `open-prs.jsonl`, commit, and stop.
