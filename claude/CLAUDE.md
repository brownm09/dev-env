...
- **Claude Code has no user-scope `settings.local.json` — only a per-project one.** Verified against the [Claude Code settings documentation](https://code.claude.com/docs/en/settings-and-configuration.md): the four settings files are User (`~/.claude/settings.json`, every project on this machine), Shared project (`.claude/settings.json`), **Project local** (`.claude/settings.local.json`, "you, in this one project only"), and Managed. There is no fifth, user-scope local file — a `~/.claude/settings.local.json` would simply never be read. Since `~/.claude/settings.json` on this machine is a symlink into dev-env's tracked, version-controlled `claude/settings.json` ([ADR-003](../docs/adr/003-config-in-version-control.md)), any Claude-Code-app-written key that embeds **project-specific** content (an auto-mode environment scan, a per-project credential hint — anything naming a specific repo's paths or policies) must go into *that project's own* `.claude/settings.local.json`, never into dev-env's tracked global file: the global file is loaded by every project's session on this machine, and the project-local file is the one real mechanism for personal, project-scoped, git-excluded state. An ordinary global preference toggle with no project-specific content (a boolean like the file's existing `theme`/`tui`/`*NotifEnabled` keys) is unaffected by this and stays in the tracked global file as before. See [ADR-139](../docs/adr/139-project-scoped-app-written-settings-belong-in-project-local.md).
