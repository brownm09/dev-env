...
**Scope boundary — this section governs *experiments*; standing *gates* are Pass 3's Gate-calibration dimension.** The calibration law is one law with two application sites: an instrument calibrated against known-good **and** known-bad references before it bears weight, and an uncalibrated check demoted to a diagnostic. What is experiment-only, and must not be imported into a gate review, is everything built around *comparing arms to reach a verdict* — tiers, the pre-registration freeze, the incumbent-influence inventory, blinding and order-randomization, the win bar, the T1–T10 sweep, and the verdict/decision vocabulary. A gate has no arms, no baseline, and no verdict; it has a threshold, and what it owes is the provenance of that threshold ([ADR-143](../docs/adr/143-gate-calibration-pass-3-dimension.md)).
