...
- **Auto-merge is off by design.** Every merge happens in-session via `gh pr merge --squash --delete-branch` after `/review`, ADR-warrant check, and doc-reconciliation check complete. Per-PR `gh pr merge --auto --squash` is permitted only after those gates pass in-session — never as fire-and-forget after `gh pr create`. Rationale: [ADR-031](../docs/adr/031-auto-merge-disabled.md).
