...
- **Merging a PR developed in a worktree:** Run `gh pr merge --squash --delete-branch` directly from the worktree. Do not call `ExitWorktree` first — it is session-bound and becomes a no-op after `/compact`, which is the common case. `gh` will fail to check out main locally and may fail to delete the local branch (the worktree holds the branch checked out), but the remote merge and remote branch delete complete successfully — those errors are benign. The local worktree directory and branch are cleaned up by the weekly `prune-merged-worktrees.py` run.
