...
- **Auto-review on PR creation.** After opening any PR, report the PR URL and instruct the user to run `/review <PR-URL> --post-comment` in a **new session** before considering the task complete. Do not run `/review` inline — the review skill is self-contained from the PR URL and running it in the same session wastes the accumulated implementation context as unnecessary input tokens. The review skill applies the `reviewed-by-claude` label automatically; a PR without that label has not been reviewed.
