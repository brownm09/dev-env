...
- **Capture post-merge follow-ups as tiles.** After `gh pr merge`, if the work surfaced any follow-ups — out-of-scope fixes, deferred work, tech debt or ideas noticed in passing — create a background-task tile for each via the `spawn_task` tool; the merge is an explicit checkpoint for this. Tiles are a low-friction capture, not a durable record: for a follow-up that must be tracked, still file an issue (the tile's spawned session can do that). Where `spawn_task` is unavailable (e.g. some terminal sessions), file a follow-up issue instead ([ADR-046](../docs/adr/046-post-merge-followup-tiles.md)).
...
per session / per PR ([ADR-055](../docs/adr/055-per-session-sharding-journal-companion-files.md)), so
