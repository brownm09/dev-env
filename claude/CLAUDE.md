...
- **Merging a PR developed in a worktree:** Call `ExitWorktree` before running `gh pr merge`. `gh pr merge --squash --delete-branch` will then find the branch unchecked-out and delete it cleanly. Skipping this causes the merge and remote branch delete to succeed (steps 1–3 of `gh`'s sequence) but the local `git branch -d` to fail (step 4) with "cannot delete branch checked out at …", leaving `gh` with a non-zero exit that looks like a full merge failure.
