...
| `_hookio.py` | PostToolUse command-output reader (`read_command_output`), merge-marker detection (incl. the two-step REST merge fallback, `is_rest_merge_command`/`output_has_rest_merge_marker`), the `scan_top_level`/`is_help_only` command-shape parsers. | 5 PostToolUse hooks + `pr-merge-reminder.py` |
