...
| `_journal_schema.py` | Shared manifest/open-PR/tile shard schema + validation (`missing_required_fields`, `missing_tile_fields`, `parse_manifest_text`, `decode_shard_bytes`). | `validate-manifest.py`, `journal-shard-write-advisory.py`, `stub-push-archive-reminder.py` |
| `_journal_shards.py` | Shared numeric-shard + legacy `open-prs.jsonl` reader (`iter_numeric_shards`, with `iter_pr_shards` / `iter_tile_shards` as named delegations; `read_legacy_entries`). | `reconcile-open-prs.py`, `post-compact.py` |
