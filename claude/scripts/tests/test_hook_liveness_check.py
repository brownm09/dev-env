...
caller-supplied `now`), `_age_desc`, and `format_warning`. `main()`'s stdin
plumbing and the real ~/.claude/scratch/hook-heartbeat/ + claude/settings.json
reads are not covered here (pure-helper convention, matching
pre-bash-drift-check.py / pre-commit-branch-check.py's own test files). The
