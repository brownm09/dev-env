...
42. **check-journal-compose-liveness test** — required when changing
    ...
    blank lines and multi-line mixed output are handled correctly, and that the abort message is
    ASCII (and therefore cp1252-safe), matching the encoding-safety convention pinned elsewhere in
    this repo's advisory-emitting scripts (items 18, 40). The script itself stays pure I/O (porcelain
    text via stdin, not a live `git status` call) so this test needs no subprocess mocking — the two
    callers (`journal-compose-with-retry.sh`'s primary check and `journal-compose/SKILL.md` Step
    0.6's defense-in-depth check) both run `git status --porcelain` themselves and pipe the output
    in ([ADR-085](docs/adr/085-journal-compose-liveness-guard.md)).
