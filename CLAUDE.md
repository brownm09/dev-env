...
**Rule:** Because `~/.claude/settings.json` is a symlink into this repo's tracked `claude/settings.json`, a Claude-Code-app-written change to that file (an auto-mode environment scan, a permission "don't ask again" approval, a `/config` toggle) becomes an uncommitted local modification in the canonical checkout — and `dev-env-sync`'s `git pull --ff-only` refuses to fast-forward past any uncommitted change to a tracked file, blocking every session on the machine until it's resolved. If the app-written content is **project-specific** (names a specific repo's paths, visibility, or policies — not a plain global preference toggle), it does not belong in this file at all: relocate it to that project's own `.claude/settings.local.json` rather than committing or discarding it here. See the global `claude/CLAUDE.md` → Platform & Environment bullet and [ADR-139](docs/adr/139-project-scoped-app-written-settings-belong-in-project-local.md) (incident: [dev-env#1049](https://github.com/brownm09/dev-env/issues/1049) / [dev-env#1050](https://github.com/brownm09/dev-env/issues/1050)).
