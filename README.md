...
| Stop | `stop-tile-enumeration-gate.py` | State-keyed post-merge tile-enumeration gate: scans the just-ended transcript and **blocks the stop (exit 2)** when a PR reached MERGED state this session by any path — including auto-merge and pure `gh api` merges the command-keyed `post-merge-tile-checkpoint.py` is blind to — but no tile-enumeration was recorded ("Follow-ups considered … → tiled/→ not tiled"; a bare "no follow-ups" does not satisfy it). The Stop-hook analog of `pre-merge-findings-gate`; complements the command-keyed hook and, unlike it, still fires in background/SDK sessions ([ADR-053](docs/adr/053-posttooluse-hooks-inert-in-background-sessions.md)). Fires once per session; honors "skip tiles" and `stop_hook_active` ([ADR-088](docs/adr/088-state-keyed-tile-enumeration-gate.md)) |
