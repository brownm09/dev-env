...
| Stop | `stop-tile-enumeration-gate.py` | State-keyed tile-enumeration gate: scans the just-ended transcript and **blocks the stop (exit 2)** on either of two independent triggers with no recorded tile-enumeration ("Follow-ups considered … → tiled/→ not tiled"; a bare "no follow-ups" does not satisfy it) — **(1)** a PR reached MERGED state this session by any path, including auto-merge and pure `gh api` merges the command-keyed `post-merge-tile-checkpoint.py` is blind to ([ADR-088](docs/adr/088-state-keyed-tile-enumeration-gate.md)), or **(2)** a `gh issue create` this session was left unresolved at Stop — not closed via a same-session merged PR's Closes/Fixes/Resolves keyword, nor explicitly closed — the pure-investigation-session case ([ADR-092](docs/adr/092-dangling-issue-tile-enumeration-gate.md)). Both are the Stop-hook analog of `pre-merge-findings-gate`; the merged-PR trigger complements the command-keyed hook and, unlike it, still fires in background/SDK sessions ([ADR-053](docs/adr/053-posttooluse-hooks-inert-in-background-sessions.md)). One recorded enumeration satisfies either or both. Fires once per session; honors "skip tiles" and `stop_hook_active` |
